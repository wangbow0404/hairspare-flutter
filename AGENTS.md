...
2. **앱 구조 파일 확인** — `~/.Codex/projects/-Users-yoram-flutter/memory/app-architecture-overview.md`
   - 화면 연결 구조, 역할별 라우트, 미연결 화면 목록을 파악하고 작업에 반영한다.

## Sequential Thinking

Follow `~/.Codex/AGENTS.md`: when the user says **단계적으로 생각해서** (or similar), use the **sequentialthinking** MCP tool before implementing.
